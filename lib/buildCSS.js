...
function buildCSS() {
  ...
  function style(s) {
    var rs = merge(s.style, s.children);
    classNames[s.className] = true;
    return b.rule('.' + s.className, rs);
  }
    ...
    if (typeof rs === 'string') {
      ...
      for (var cssKey in rs) {
        ...
        if (typeof cssValue === 'object') {
          ...
          if (cssValue instanceof Style) {
            if (pseudoSelectors[cssKey]) {
              cssValue = merge(cssValue.style, cssValue.children);
            } else {
              deferred.push(cssValue);
              continue;
            }
          }
